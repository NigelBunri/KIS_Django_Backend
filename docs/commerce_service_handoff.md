...
- **Tests executed**:
  - `python3 manage.py test apps.commerce.tests.ServiceBookingAPITests` – fails immediately because the default PostgreSQL `DATABASE_URL` points at `10.14.20.99:5432`, which is unreachable inside this sandbox (OperationalError).
  ...
  - `DATABASE_URL=postgresql://kis_dev_user@10.14.20.99:5432/kis_test python3 manage.py migrate` – the sandbox refuses the Postgres connection, raising `OperationalError: connection is bad: connection to server at "10.14.20.99", port 5432 failed: Operation not permitted`. Postgres is therefore not reachable from this session, blocking migrations/tests on the requested database.
...
- **PostgreSQL provisioning note**: `initdb -D /tmp/kis_pgdata` fails here because the sandbox cannot allocate shared memory segments (the `shmget` call is denied). The `pg_ctl` server also cannot start, so a container or an external Postgres host is required. Future sessions should run these steps on a host that allows shared memory:
  ...
  4. `DATABASE_URL=postgresql://kis_dev:hunter2@10.14.20.99:5433/kis_test_db python3 manage.py migrate` to confirm the migrations apply.
