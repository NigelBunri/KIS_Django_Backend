...
def seed_fixed_engines(apps, schema_editor):
    EngineRegistry = apps.get_model("health_ops", "EngineRegistry")
    EngineStepDefinition = apps.get_model("health_ops", "EngineStepDefinition")
    ...
    for code, name, category, step_count in ENGINE_DEFINITIONS:
        ...
        blueprints = ENGINE_STEP_BLUEPRINTS.get(code) or [
            (f"step_{idx + 1}", f"Step {idx + 1}", f"Guided step {idx + 1} for {name}.")
        ...
        for idx, (step_key, step_title, step_description) in enumerate(blueprints, start=1):
...
def noop_reverse(apps, schema_editor):
    # Keep seeded engines/steps during rollback to avoid breaking live references.
    return
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("health_ops", "0009_videoengineitem_enginesession_expired_at_and_more"),
    ]

    operations = [
        migrations.RunPython(seed_fixed_engines, noop_reverse),
    ]
