...
class DeviceQRToken(models.Model):
    """
    One-time token used to link a secondary session to a user account —
    either another PHONE (phone_link, scanned QR only) or a WEB browser
    (web_login, scannable QR *or* a short code typed into a form).
    ...
    Flow (web_login):
      1. Parent (primary) device calls GET /auth/devices/web-pairing/ → server
         creates a short-lived, short human-typeable code, returned as both a
         QR payload (a pairing URL) and plain code text.
      2. On the website, the user either opens the scanned link or types the
         code into a form, which POSTs it to /auth/devices/web-pairing/redeem/.
      3. Server validates + consumes the code and issues a normal device-bound
         JWT session for a new platform="web" Device, WITHOUT revoking any
         other web session — a primary device may have many concurrent web
         sessions. Only ever generated by a parent device (secondary devices
         are refused at generation time, satisfying "0 web sessions" for them).
    """
    PURPOSE_PHONE_LINK = 'phone_link'
    PURPOSE_WEB_LOGIN = 'web_login'
    PURPOSE_CHOICES = [
        (PURPOSE_PHONE_LINK, 'Phone link'),
        (PURPOSE_WEB_LOGIN, 'Web login'),
    ]

    # Excludes visually-ambiguous characters (0/O, 1/I/L) so a human can
    # reliably read and type the code back in.
    _WEB_CODE_ALPHABET = 'ABCDEFGHJKMNPQRSTUVWXYZ23456789'
    _WEB_CODE_LENGTH = 10
    _WEB_CODE_TTL = datetime.timedelta(minutes=10)
    _PHONE_LINK_TTL = datetime.timedelta(hours=3)
    ...
    purpose = models.CharField(max_length=20, choices=PURPOSE_CHOICES, default=PURPOSE_PHONE_LINK)
    # HMAC-SHA256 hex of the plain token/code (never stored plain).
    ...
    @classmethod
    def generate_for_device(cls, user, device, purpose: str = PURPOSE_PHONE_LINK) -> tuple:
        """
        Always create a fresh token for this (user, device, purpose) pair.
        Deletes any previously unused tokens for the same device + purpose first.
        Returns (DeviceQRToken instance, token_plain).
        token_plain is returned ONCE here; it is never stored.
        """
        cls.objects.filter(
            user=user,
        ...
        if purpose == cls.PURPOSE_WEB_LOGIN:
            token_plain = ''.join(secrets.choice(cls._WEB_CODE_ALPHABET) for _ in range(cls._WEB_CODE_LENGTH))
            ttl = cls._WEB_CODE_TTL
        else:
            token_plain = secrets.token_urlsafe(48)
            ttl = cls._PHONE_LINK_TTL
    ...
    @classmethod
    def consume(cls, token_plain: str, purpose: str = PURPOSE_PHONE_LINK):
        """
        Validate and consume a plain token/code posted by the redeeming side.
        Returns the DeviceQRToken instance on success, or None if invalid/expired/used
        /wrong purpose. Sets used_at on success (caller should set used_by_device and save).
        """
        if not token_plain:
            return None
        # Web codes are typed by hand — normalize case/whitespace so
        # "ab3d-fg7h-2k" and "AB3D-FG7H-2K" both match what was hashed at
        # generation time (generated codes are always uppercase).
        normalized = str(token_plain).strip().upper().replace('-', '').replace(' ', '') \
            if purpose == cls.PURPOSE_WEB_LOGIN else token_plain
        token_hash = hmac.new(
            settings.SECRET_KEY.encode('utf-8'),
        ...
        # racing the same code can both pass a plain .get() before either
